...
public class mobs implements entities {
	...
	public void move(String mobs, player p, CollisionManager check, MapLoader map) {
		...
		if (check.checkCollision(this, "down", map) != 0) {
			if (p.getOx() < getOx()) {
				...
				if (check.checkCollision(this, "left", map) == 0) {
					...
					if (check.checkCollision(this, "down", map) == 0) {
						x += speed;
						// giữ nguyên animation run (nếu muốn có thể đổi thành idle)
					}
				}
			} else {
				// chạy sang phải
	...
	public void Attack(String mobs, player p, int d, int range) { // d: hoạt ảnh gây sát thương
		...
		AnimationFrameData P_frames = new AnimationFrameData("player", "attack");
	...
	public void die(String mobs) {
		if (!isDying) {
			setAnimation(mobs, "die");
	...
	public void gotHit(String mobs) {
		if (!isHit) {
			setAnimation(mobs, "gotHit");
			frameIndex = frameStart;
			isHit = true;
		}
	}

//	public void Jump(String mobs) {
//		setAnimation(mobs, "Jump");
//
//	}
